...
| M9 | 热路径调试残留数（E） | 两步：① Grep 候选 `getenv|fprintf|console.log|debugger;|System.out.print|IsBadReadPtr|debug &&|print(`；② **排除产品意图输出**（CLI stdout、结构化 logger、带文档的 getenv 配置）后计**净数**（坐标特判、魔法偏移、孤立 debug 打印、热路径 traceback.print_exc） | D4/G4 |
...
| G4 AI 排查残留混入热路径 | 热路径调试残留 ≥5 处（M9 排除产品意图后的净数） | 45 |
...
**信号计数口径**：每类别命中 ≥1 处计 1 信号（单类别多处仍计 1，但超额直接触发 45 档）；**人类正常债务型**= 非系统性 CP、best-effort 型空 catch、DZ、TODO、个别大文件；**AI 过程指纹**= M、E、W、系统性 CP、G1 命中、H、吞真实错误的空 catch；分类模糊按 AI 过程指纹（保守）。
...
| 任一命中：M≥3 / E≥3 / W≥2 / 系统性 CP≥3 / C≥5 / H≥3 / CD≥3 / **G1 命中** / 1 处指纹+其他信号 / 总信号 ≥5 | 45 | 55 |
| 泛滥：M 遍布核心 / E≥5（→G4）/ CP 系统性 ≥6 组 / W≥4 / H≥5 | 20 | 30 |
