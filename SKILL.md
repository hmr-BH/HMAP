...
Score each dimension 0-100, weight-sum, then apply structural hard gates. Semantic coherence (names telling the truth, contracts closed, concepts single-sourced, vocabulary self-evident) is audited by the fixed SCA protocol (M14) and feeds all four dimensions — it is the third evaluation axis alongside structure and residue. **Detailed rules (M1-M16 measurement table, tier mapping, SCA semantic-coherence protocol, RM responsibility-mixing protocol, tie-breaks): [`references/scoring-rubric.md`](references/scoring-rubric.md); signal catalog: [`references/ai-slop-signals.md`](references/ai-slop-signals.md); worked scoring example: [`references/golden-example.md`](references/golden-example.md).**
...
- **Measure first (mandatory before scoring)**: run M1-M16 from the rubric's global measurement table and fill the sheet. **Measured values decide tiers — before impressions.**
...
Scope: N source files, M read (depth: light/medium/deep), confidence: high/medium/low. Measurement sheet: key M1-M16 values (≤5 lines).
